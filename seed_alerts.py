...
# ML-алерты (source=ml_anomaly/ml_forecast) здесь НЕ создаются. Раньше тут была
# статичная заглушка ML_DEMO (3 захардкоженных алерта — витрина ML-секции на Render).
# Снята в Фазе 4 инкр.3: реальные ML-алерты теперь считает entrypoint на Render,
# вызывая generate_telemetry → detect_anomalies/forecast_telemetry --write-alerts
# по живой телеметрии. См. docs/plans/phase4/03_render_inference.md.
